...
13. if length(generators) != L + 1, return INVALID
14. (Q_1, MsgGenerators) = generators
15. (H_1, ..., H_L) = MsgGenerators
16. (H_j1, ..., H_jU) = (MsgGenerators[j1], ..., MsgGenerators[jU])
...
1.  domain = calculate_domain(PK, generators, header, api_id)
2.  random_scalars = calculate_random_scalars(3+U)
3.  (r1, r2, r3, m~_j1, ..., m~_jU) = random_scalars
4.  B = P1 + Q_1 * domain + H_1 * msg_1 + ... + H_L * msg_L
5.  Abar = A * r1
6.  Bbar = B * r1 - Abar * e
7.  T =  Abar * r2 + Bbar * r3 + H_j1 * m~_j1 + ... + H_jU * m~_jU
8.  c = calculate_challenge(Abar, Bbar, T, (i1, ..., iR),
                             (msg_i1, ..., msg_iR), domain, ph, api_id)
9.  r4 = - r1^-1 (mod r)
10. r2^ = r2 + e * r4 * c (mod r)
11. r3^ = r3 + r4 * c (mod r)
12. for j in (j1, ..., jU): m^_j = m~_j + msg_j * c (mod r)
13. proof = (Abar, Bbar, r2^, r3^, (m^_j1, ..., m^_jU), c)
14. return proof_to_octets(proof)
```
...
14. (Q_1, MsgGenerators) = generators
15. (H_1, ..., H_L) = MsgGenerators
16. (H_i1, ..., H_iR) = (MsgGenerators[i1], ..., MsgGenerators[iR])
17. (H_j1, ..., H_jU) = (MsgGenerators[j1], ..., MsgGenerators[jU])
...
4. T = T + D * c
5. cv = calculate_challenge(Abar, Bbar, T, (i1, ..., iR),
                            (msg_i1, ..., msg_iR), domain, ph, api_id)
6. if c != cv, return INVALID
...
```
challenge = calculate_challenge(Abar, Bbar, C, i_array, msg_array,
                                                     domain, ph, api_id)
